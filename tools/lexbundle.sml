(* to build ml-lex+, feed me to sml, e.g., sml < lexbundle.sml *)
...
*
 * $Log: lexgen.sml.109-23+,v $
 * Revision 1.1  1997/06/10 00:47:02  nr
 * Initial revision
 *
 ...
 *
	04/12/96 (Norman Ramsey <nr@cs.purdue.edu>) :
		Added %decl, to insert declarations between YYBEGIN and rules code.
...
structure LexGen: LEXGEN =
   ...
   datatype token = CHARS of bool array | QMARK | STAR | PLUS | BAR
	  ...
	  | BOF | EOF | ASSIGN | SEMI | ARROW | LEXMARK | LEXSTATES  |
	    COUNT | REJECT | FULLCHARSET | STRUCT | HEADER | ARG | INNER
		 ...
		 of #"%" => (case nextch() of 
			...
			| a => let fun f s =
				...
				in if command = "reject" then REJECT
				   ...
				   else if command = "inner" then INNER
...
fun lexGen(infile) =
    ...
    let val sayln = fn x => (say x; say "\n")
	 ...
	 sayln "  let";
	 say "      fun scan (s,AcceptingLeaves : Internal.yyfinstate";
